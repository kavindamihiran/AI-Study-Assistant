...
async def get_document(document_id: str, request: Request) -> dict:
    document = _store(request).get_document(
        document_id, user_id=current_auth(request).user_id
    )
    if document is None:
...
async def delete_document(document_id: str, request: Request) -> None:
    if not await _store(request).delete(
        document_id, user_id=current_auth(request).user_id
    ):
        raise HTTPException(status_code=404, detail="Document not found")
...
async def reindex_document(document_id: str, request: Request) -> dict:
    document = await _store(request).reindex(
        document_id, user_id=current_auth(request).user_id
    )
    if document is None:
        raise HTTPException(status_code=404, detail="Document not found")
